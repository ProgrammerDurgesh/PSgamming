		...
		<div class="container">
			...
			<div class="row">
				<div class="col-md-4 mx-auto">
					...
					<div class="faq-form">
						<form id="otpForm" action="${pageContext.request.contextPath}/otp"
							method="post">
							<div class="row">
								<div class="form-group col-md-12">
									<input type="hidden" class="form-control" value=<%= request.getAttribute("email") %>
										name="email" id="email" placeholder="Enter email"> <label>OTP</label>
									<input type="text" class="form-control" name="otp" id="otp"
										placeholder="Enter otp"> <label class="frmValidmsg"
										for="">Please eneter OTP</label>
								</div>
							</div>
							<div class="row">
		...
		$(document).ready(function() {
		  ...
		  function logout() {
			...
			function validateOtp() {
				...
				if (otp.length !== 6) {
					isValid = false;
					$("input[name=otp]").next(".frmValidmsg").show();
				}
				// hidden email  
				var email = $("input[name=email]").val().trim();
				console.log("the emai value is:",email);
				return isValid;
			}
			// Validate form on submit
			$("#otpForm").on("submit", function(ev) {
				ev.preventDefault();
				if (validateOtp()) {
